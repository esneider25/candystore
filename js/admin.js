// ============================================================
// CandyStore Admin Panel Logic (Enhanced)
// ============================================================
    ...
    <div class="admin-container">
      <aside class="admin-sidebar">
        <div class="admin-logo">
          <span class="admin-logo-icon" style="font-size: 1.8rem; margin-right: 8px;">🍭</span>
          <span class="admin-logo-text">CandyStore</span>
          <span style="font-size: 1.1rem; margin-left: 5px; color: var(--text-secondary); font-weight: 500;">Admin</span>
        </div>
        <ul class="admin-nav">
...
function exportOrders() {
  ...
  link.download = `recargaCandyStore_pedidos_${new Date().toISOString().slice(0, 10)}.csv`;
  link.click();
...
async function testTelegramConnection() {
  ...
  const success = await sendTelegramMessage('🤖 <b>¡Conexión Exitosa!</b>\nLas notificaciones de CandyStore están funcionando correctamente.');
  if (success) {
...
function renderSettings(container) {
  ...
  try {
    ...
    const defaultTerms = [
      { title: 'Aceptación del Servicio', titleColor: '#0ea5e9', desc: 'Al utilizar CandyStore, registrarte o realizar un pedido, aceptas estar de acuerdo con todos los términos aquí descritos. Nos reservamos el derecho de modificar estos términos en cualquier momento.', descColor: '#e2e8f0' },
      { title: 'Responsabilidad de Datos (IDs y Cuentas)', titleColor: '#facc15', desc: 'El cliente es el único responsable de proporcionar correctamente su ID de jugador, Zona o datos de cuenta. CandyStore no se hace responsable por recargas enviadas a cuentas equivocadas debido a errores tipográficos por parte del usuario.', descColor: '#e2e8f0' },
      ...
      <div class="modal">
        ...
        <div class="form-group" style="margin-top: 15px;">
          ...
          <select id="role-select" class="form-input" onchange="document.getElementById('discount-group').style.display = this.value === 'revendedor' ? 'block' : 'none'; document.getElementById('referral-limit-group').style.display = this.value === 'influencer' ? 'block' : 'none'">
            ...
            <option value="influencer" ${currentRole === 'influencer' ? 'selected' : ''}>Influencer CandyStore</option>
...
function renderLanding(container) {
  ...
  container.innerHTML = `
    ...
    <!-- 3. ¿Por qué CandyStore? (Ventajas) -->
    <div class="admin-card" style="margin-bottom: 20px;">
      <h2 class="admin-card-title">¿Por qué CandyStore? (Ventajas)</h2>
