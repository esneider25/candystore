// ============================================================
// CandyStore — Main App Logic & SPA Routing (v2 + Orders)
// ============================================================
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (urlParams.get('ref')) {
    localStorage.setItem('recargaCandyStore_referredBy', urlParams.get('ref'));
  }
  ...
  if (localStorage.getItem('recargaCandyStore_theme') === 'light') {
...
function showAnnouncementModal(message) {
  if (sessionStorage.getItem('recargaCandyStore_announcement_seen') === 'true') return;
  ...
  document.getElementById('announcement-modal-btn').addEventListener('click', () => {
    sessionStorage.setItem('recargaCandyStore_announcement_seen', 'true');
    const overlay = modalContainer.querySelector('.modal-overlay');
    overlay.classList.remove('active');
    setTimeout(() => {
...
function renderApp() {
  ...
  if (config.maintenance) {
    app.innerHTML = `
      <div class="bg-candy-world" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: -1;">${typeof renderBubbles === 'function' ? renderBubbles() : ''}</div>
  ...
  const termsAccepted = sessionStorage.getItem('recargaCandyStore_terms_accepted');
  const termsHtml = !termsAccepted ? (typeof renderTermsModal === 'function' ? renderTermsModal() : '') : '';

  if (appState.currentView === 'home') {
    app.innerHTML = `
      <div class="bg-candy-world">${renderBubbles()}</div>
      ${renderNavbar()}
    ...
    app.innerHTML = `
      <div class="bg-candy-world">${typeof renderBubbles === 'function' ? renderBubbles() : ''}</div>
...
function applyDiscount() {
  ...
  if (!discount) {
    showToast('?? Código inválido, expirado o límite excedido');
    appState.appliedDiscount = null;
    updateOrderSummary();
    return;
  }
...
async function _submitOrderLogic() {
  ...
  if (productType === 'game-id') {
    ...
    if (!uidInput || !uidInput.value.trim()) {
      showToast('?? Ingresa tu ID del juego');
      uidInput?.focus();
      return;
    }
    if (typeof userProfile !== 'undefined' && userProfile && userProfile.role === 'revendedor') {
      ...
      if (ids.length === 0) {
        showToast('?? Ingresa al menos un ID');
        return;
      }
      if (ids.length > 10) {
        showToast('?? Máximo 10 IDs permitidos por pedido masivo');
        return;
      }
      gameId = ids;
    ...
    if (!uidInput || !uidInput.value.trim()) {
      showToast('?? Ingresa el Player ID');
      uidInput?.focus();
      return;
    }
    if (!zoneInput || !zoneInput.value.trim()) {
      showToast('?? Ingresa el Zone ID');
      zoneInput?.focus();
      return;
    }
    ...
    if (!emailInput || !emailInput.value.trim()) {
      showToast('?? Ingresa el correo o usuario de la cuenta');
      emailInput?.focus();
      return;
    }
    if (!passInput || !passInput.value.trim()) {
      showToast('?? Ingresa la contraseña de la cuenta');
      passInput?.focus();
      return;
    }
  ...
  if (appState.selectedPackageIndex === null) {
    showToast('?? Selecciona un paquete');
    return;
  }
  ...
  if (appState.selectedPaymentId === 'wallet') {
    if (typeof currentUser === 'undefined' || !currentUser) {
      showToast('?? Debes iniciar sesión para usar tu monedero');
      return;
    }
    ...
    if (currentWallet < finalUsd) {
      showToast(`?? Saldo insuficiente. Tienes $${currentWallet.toFixed(2)} USD y necesitas $${finalUsd.toFixed(2)} USD.`);
      return;
    }
  } else if (!appState.selectedScreenshot) {
    showToast('?? Sube la captura del comprobante de pago');
        ...
        try {
          ...
          if (pollData.ok && (estadoStr === 'completado' || estadoStr === 'completed')) {
            ...
            if (errorLower.includes('ya fue usado') || errorLower.includes('ya existe') || errorLower.includes('already used')) {
              ...
              setStatus('invalid-id', `Verifica que el ID o la cuenta sean correctos. El sistema rechazó la recarga. (${errorMsg})`);
              if (typeof sendTelegramMessage === 'function') {
                sendTelegramMessage(`?? <b>DATOS INVÁLIDOS — #${order.id}</b>\n\nEl proveedor rechazó el pedido luego de procesar. El cliente debe corregir los datos. Mensaje de error: ${errorMsg}`);
              }
            }
          }
        } catch (e) {
...
function submitWalletRecharge() {
  if (!currentUser) {
    showToast('?? Debes iniciar sesión para recargar tu monedero');
    return;
  }
  const amount = appState.selectedPackageIndex;
  const method = PAYMENT_METHODS.find(m => m.id === appState.selectedPaymentId);
  
  if (!amount) { showToast('?? Selecciona un monto'); return; }
  if (!method) { showToast('?? Selecciona un método de pago'); return; }
  if (!appState.selectedScreenshot) {
    showToast('?? Sube la captura del comprobante');
    return;
  }
        ...
        <div class="payment-flow-steps" id="payment-flow-steps">
          ...
          <div class="pf-step-content" id="pf-content-2">
            <div class="pf-processing">
              ...
              <span class="pf-processing-icon">??</span>
            </div>
...
async function lookupOrder() {
  ...
  if (!input || !input.value.trim()) {
    showToast('?? Ingresa un dato de búsqueda');
    input?.focus();
    return;
  }
...
function rectifyOrderId(orderId) {
  ...
  if (order.productType === 'account') {
    ...
    if (!idInput || !idInput.value.trim() || !zoneInput || !zoneInput.value.trim()) {
      showToast('?? Ingresa el ID y la Zona');
      return;
    }
    ...
    if (!input || !input.value.trim()) {
      showToast('?? Ingresa los datos correctos');
      input?.focus();
      return;
    }
    ...
    messagesContainer.innerHTML = `
      <div class="support-msg support-msg--bot">
        <div class="support-msg-bubble">
          ¡Hola! ?? Bienvenido a <strong>CandyStore</strong>. ¿En qué puedo ayudarte hoy?
        </div>
...
function startSupportSession() {
  ...
  if (msgs.length === 0) {
     addMessage(sessionId, 'bot', '¡Hola! ?? Bienvenido a CandyStore. ¿En qué puedo ayudarte hoy?', contact);
  } else {
...
function previewScreenshot(input) {
  ...
  if (file.size > 5 * 1024 * 1024) {
    showToast('?? La captura no debe superar los 5MB');
    input.value = '';
    return;
  }
...
window.verifyGameId = async function(productId) {
  ...
  if (!uidInput || !uidInput.value.trim()) {
    showToast('?? Ingresa el ID del juego primero.', 'info');
    uidInput?.focus();
    return;
  }
...
function initPublicAuth() {
  ...
  firebase.auth().onAuthStateChanged(async (user) => {
    ...
    if (user && user.email === 'admin@CandyStore.com') {
       const adminHtml = `
            <div style="display:flex; gap:8px; align-items:center;">
              <a onclick="window.location.href='https://admin.CandyStore.com/'" class="nav-cta" style="background: linear-gradient(135deg, #0ea5e9, #0284c7); cursor:pointer; padding: 8px 16px;">Ir al Panel</a>
...
function registerWithEmail() {
  ...
  const email = document.getElementById('auth-email').value.trim();
  const pass = document.getElementById('auth-pass').value.trim();
  if(!name || !email || !pass) return showToast('?? Llena todos los campos');
  
  firebase.auth().createUserWithEmailAndPassword(email, pass).then(result => {
    const user = result.user;
    user.updateProfile({ displayName: name });
    
    const referredBy = localStorage.getItem('recargaCandyStore_referredBy') || null;
...
function authWithGoogle() {
  ...
  firebase.auth().signInWithPopup(provider).then((result) => {
    ...
    const user = result.user;
    if (user.email === 'admin@CandyStore.com') return; // Admin bypass
...
function renderDashboardSavedIds() {
  ...
  container.innerHTML = savedIds.map((item, index) => `
    <div style="background: rgba(255,255,255,0.05); padding: 10px; border-radius: 8px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center; border: 1px solid rgba(255,255,255,0.1);">
      ...
      <button onclick="deleteSavedId(${index})" style="background:none; border:none; color: #ff5252; cursor:pointer; font-size: 1.2rem;" title="Eliminar">???</button>
    </div>
  `).join('');
}
...
async function saveNewId() {
  ...
  if (!game || !uid) {
    showToast('?? Debes ingresar el nombre del juego y el ID');
    return;
  }
...
// Initial App Render Trigger
window.addEventListener('DOMContentLoaded', () => {
  if (window.DATA_LOADED && typeof renderApp === 'function') {
    renderApp();
  }
});
